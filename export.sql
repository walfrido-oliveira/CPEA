...
FROM guiding_parameter_values
    ...
    LEFT JOIN unities u2 ON u2.id = guiding_parameter_values.unity_legislation_id
    LEFT JOIN unities u3 ON u3.id = guiding_parameter_values.unity_analysis_id;
...
FROM `parameter_analyses`
    LEFT JOIN analysis_parameters ON analysis_parameters.id = parameter_analyses.analysis_parameter_id
    LEFT JOIN parameter_analysis_groups ON parameter_analysis_groups.id = parameter_analyses.parameter_analysis_group_id;
...
FROM guiding_parameters
    LEFT JOIN environmental_areas ON environmental_areas.id = guiding_parameters.environmental_area_id
    LEFT JOIN environmental_agencies ON environmental_agencies.id = guiding_parameters.environmental_agency_id


SELECT *
FROM analysis_results
    LEFT JOIN analysis_orders ON analysis_orders.id = analysis_results.analysis_order_id
WHERE analysis_orders.campaign_id=269;
