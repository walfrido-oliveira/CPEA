                ...
                <div class="flex mt-4">
                    <table class="table table-responsive md:table w-full">
                        ...
                        <tbody id="environmental-area_table_content">
                            @include('environmental-area.filter-result', ['environmental-area' => $environmentalAreas])
                        </tbody>
                    </table>
                </div>
    ...
    <script>
        window.addEventListener("load", function() {
            ...
            function eventsDeleteCallback() {
                document.querySelectorAll('.delete_environmental_area').forEach(item => {
                item.addEventListener("click", function() {
                    if(this.dataset.type != 'multiple') {
                        var url = this.dataset.url;
                        var modal = document.getElementById("delete__environmental_area_modal");
                        ...
                        var urls = '';
                        document.querySelectorAll('input:checked.environmental_area_url').forEach((item, index, arr) => {
                            urls += item.value ;
