<x-app-layout>
        ...
        <div class="max-w-6xl mx-auto px-4">
            <div class="flex md:flex-row flex-col">
                ...
                <div class="w-full flex justify-end">
                    ...
                    <div class="m-2">
                        <a class="btn-outline-warning" href="{{ route('registers.environmental-area.edit', ['environmental_area' => $environmental_area->id]) }}">{{ __('Editar') }}</a>
                    </div>
                    <div class="m-2">
                        <button type="button" class="btn-outline-danger delete-environmental_area" id="environmental_area_delete" data-toggle="modal" data-target="#delete_modal" data-id="{{ $environmental_area->id }}">{{ __('Apagar') }}</button>
                    </div>
                </div>
            </div>
    ...
    <x-modal title="{{ __('Excluir usuário') }}"
             ...
             method="DELETE"
             url="{{ route('registers.environmental-area.destroy', ['environmental_area' => $environmental_area->id]) }}"
             redirect-url="{{ route('registers.environmental-area.index') }}"/>

    <script>
        function eventsDeleteCallback() {
            document.querySelectorAll('.delete-environmental_area').forEach(item => {
