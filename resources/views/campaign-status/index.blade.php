<x-app-layout>
    <div class="py-6 index-campaign-status">
        <div class="md:max-w-6xl lg:max-w-full mx-auto px-4">
            ...
            <div class="flex md:flex-row flex-col">
                ...
                <div class="w-full flex justify-end">
                    <div class="m-2 ">
                        <a class="btn-outline-info" href="{{ route('registers.campaign-status.create') }}" >{{ __('Cadastrar') }}</a>
                    </div>
                    <div class="m-2">
                        <button type="button" class="btn-outline-danger delete-campaign-status" data-type="multiple">{{ __('Apagar') }}</a>
                    </div>
                </div>
            </div>
            ...
            <div class="py-2 my-2 bg-white rounded-lg min-h-screen">
                ...
                <div class="flex mt-4">
                    <table id="campaign_status_table" class="table table-responsive md:table w-full">
                        @include('campaign-status.filter-result', ['campaignStatuses' => $campaignStatuses, 'ascending' => $ascending, 'orderBy' => $orderBy])
                    </table>
                </div>
                <div class="flex mt-4 p-2" id="pagination">
                    {{ $campaignStatuses->appends(request()->input())->links() }}
                </div>
            </div>
        </div>
    </div>

    <x-modal title="{{ __('Excluir Status Campanha') }}"
             msg="{{ __('Deseja realmente apagar esse Status Campanha?') }}"
    ...
    <script>
        window.addEventListener("load", function() {
            var filterCallback = function (event) {
                ...
                var url = "{!! route('registers.campaign-status.filter') !!}";
                ...
                ajax.onreadystatechange = function() {
                    if (this.readyState == 4 && this.status == 200) {
                        ...
                        document.getElementById("campaign_status_table").innerHTML = resp.filter_result;
            ...
            function eventsFilterCallback() {
                ...
                document.querySelectorAll("#campaign_status_table thead [data-name]").forEach(item => {
            ...
            function eventsDeleteCallback() {
                ...
                item.addEventListener("click", function() {
                    if(this.dataset.type != 'multiple') {
                        ...
                        var urls = '';
                        document.querySelectorAll('input:checked.campaign-status-url').forEach((item, index, arr) => {
                            urls += item.value ;
