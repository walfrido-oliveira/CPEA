    ...
    <div class="w-full md:w-1/2 flex justify-end">
      <button type="button" class="btn-transition-primary edit-coordinate px-1" title="Editar Coodernada" style="margin-top: 0.2rem;">
        <svg xmlns="http://www.w3.org/2000/svg" class="h-8 w-8 text-yellow-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-wiph="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z">
          </path>
        </svg>
      </button>
      ...
      <button type="button" class="btn-transition-primary add-coordinates px-1" title="Adicionar Linhas"
        @if(!isset($formValue)) style="display: none" @endif>
  ...
  <div class="my-2 px-3">
    <table class="table table-responsive md:table w-full">
      ...
      <tbody id="table_coordinates">
        ...
        <tr data-row="{{ $key }}">
          <td>
            <x-jet-input readonly="true" id="point" class="form-control block mt-1 w-full" type="text"
              value="{{ isset($coordinate['point']) ? $coordinate['point'] : '' }}"
              name="{{ 'coordinates[' . $key . '][point]' }}" />
          </td>
          <td>
            <x-jet-input readonly="true" id="point" class="form-control block mt-1 w-full" type="text"
              value="{{ isset($coordinate['zone']) ? $coordinate['zone'] : '' }}"
              name="{{ 'coordinates[' . $key . '][zone]' }}" />
          </td>
          <td>
            <x-jet-input readonly="true" id="point" class="form-control block mt-1 w-full" type="text"
              value="{{ isset($coordinate['me']) ? $coordinate['me'] : '' }}"
              name="{{ 'coordinates[' . $key . '][me]' }}" />
          </td>
          <td>
            <x-jet-input readonly="true" id="point" class="form-control block mt-1 w-full" type="text"
              value="{{ isset($coordinate['mn']) ? $coordinate['mn'] : '' }}"
