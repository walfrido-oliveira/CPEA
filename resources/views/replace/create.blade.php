<x-app-layout>
    <div class="py-6 create-replace">
        <div class="md:max-w-6xl lg:max-w-full mx-auto px-4">
            <form method="POST" action="{{ route('registers.replace.store') }}">
                @csrf
                @method("POST")
                <div class="flex md:flex-row flex-col">
                    <div class="w-full flex items-center">
                        <h1>{{ __('Cadastrar De Para') }}</h1>
                    </div>
                ...
                <div class="py-2 my-2 bg-white rounded-lg min-h-screen">
                    ...
                    <div class="flex flex-wrap mx-4 px-3 py-2 mt-4">
                        <div class="w-full px-3 mb-6 md:mb-0">
                            ...
                            <x-jet-input id="from" class="form-control block mt-1 w-full" type="text" name="from" maxlength="255" required autofocus autocomplete="from" />
                        </div>
                    </div>
                    <div class="flex flex-wrap mx-4 px-3 py-2 mt-4">
                        <div class="w-full px-3 mb-6 md:mb-0">
                            <x-jet-label for="to" value="{{ __('Para') }}" required/>
                            <x-custom-select :options="$to" name="to" id="to" value=""/>
