<div class="inline-block relative w-full {{ $attributes['class'] }}">
    <select class="{{ $attributes['select-class'] }} block w-full custom-select focus:outline-none focus:shadow-outline @if(!isset($attributes['no-filter'])) filter-field @endif" {{ $attributes }}>
