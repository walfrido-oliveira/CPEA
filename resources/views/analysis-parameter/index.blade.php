<x-app-layout>
    <div class="py-6 index-analysis-parameter">
        <div class="md:max-w-6xl lg:max-w-full mx-auto px-4">
            ...
            <div class="flex md:flex-row flex-col">
                ...
                <div class="w-full flex justify-end">
                    <div class="m-2 ">
                        <a class="btn-outline-info" href="{{ route('registers.analysis-parameter.create') }}" >{{ __('Cadastrar') }}</a>
                    </div>
                    <div class="m-2">
                        <button type="button" class="btn-outline-danger delete-analysis-parameter" data-type="multiple">{{ __('Apagar') }}</a>
                    </div>
                </div>
            </div>
            ...
            <div class="py-2 my-2 bg-white rounded-lg min-h-screen">
                ...
                <div class="flex mt-4">
                    <table id="analysis_parameter_table" class="table table-responsive md:table w-full">
                        @include('analysis-parameter.filter-result', ['analysisParameters' => $analysisParameters, 'ascending' => $ascending, 'orderBy' => $orderBy])
                    </table>
                </div>
                <div class="flex mt-4 p-2" id="pagination">
                        {{ $analysisParameters->appends(request()->input())->links() }}
                </div>
            </div>
        </div>
    </div>
    ...
    <script>
        window.addEventListener("load", function() {
            var filterCallback = function (event) {
                ...
                var url = "{!! route('registers.analysis-parameter.filter') !!}";
                ...
                ajax.onreadystatechange = function() {
                    if (this.readyState == 4 && this.status == 200) {
                        ...
                        document.getElementById("analysis_parameter_table").innerHTML = resp.filter_result;
            ...
            function eventsFilterCallback() {
                ...
                document.querySelectorAll("#analysis_parameter_table thead [data-name]").forEach(item => {
