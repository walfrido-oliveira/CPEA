<x-app-layout>
    <div class="py-6 index-analysis-method">
        <div class="md:max-w-6xl lg:max-w-full mx-auto px-4">
            ...
            <div class="flex md:flex-row flex-col">
                ...
                <div class="w-full flex justify-end">
                    <div class="m-2 ">
                        <a class="btn-outline-info" href="{{ route('registers.analysis-method.create') }}" >{{ __('Cadastrar') }}</a>
                    </div>
                    <div class="m-2">
                        <button type="button" class="btn-outline-danger delete-analysis-method" data-type="multiple">{{ __('Apagar') }}</a>
                    </div>
                </div>
            </div>
            ...
            <div class="pb-2 my-2 bg-white rounded-lg min-h-screen">
                ...
                <div class="flex mt-4">
                    <table id="analysis_method_table" class="table table-responsive md:table w-full">
                        @include('analysis-method.filter-result', ['analysisMethods' => $analysisMethods, 'ascending' => $ascending, 'orderBy' => $orderBy])
                    </table>
                </div>
                <div class="flex mt-4 p-2" id="pagination">
                    {{ $analysisMethods->appends(request()->input())->links() }}
                </div>
            </div>
        </div>
    </div>
    ...
    <script>
        window.addEventListener("load", function() {
            var filterCallback = function (event) {
                ...
                var url = "{!! route('registers.analysis-method.filter') !!}";
                ...
                ajax.onreadystatechange = function() {
                    if (this.readyState == 4 && this.status == 200) {
                        ...
                        document.getElementById("analysis_method_table").innerHTML = resp.filter_result;
            ...
            function eventsFilterCallback() {
                ...
                document.querySelectorAll("#analysis_method_table thead [data-name]").forEach(item => {
            ...
            function eventsDeleteCallback() {
                ...
                item.addEventListener("click", function() {
                    if(this.dataset.type != 'multiple') {
                        ...
                        var urls = '';
                        document.querySelectorAll('input:checked.analysis-method-url').forEach((item, index, arr) => {
                            urls += item.value ;
